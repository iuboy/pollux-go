...
pollux↔Tongsuo PSK resume / 0-RTT 已打通：

- **标准 stateless-ticket 模型**：无状态 ticket（TEK 加密的 PSK 句柄）+ PSK 派生
  （`resumption_master_secret` + `ticket_nonce`）。
- **binder wire 语义（commit 86a2b624）**：binder transcript 截断到 `binders_len`
  字段前（identities 含，binders 不含），`pre_shared_key` 的 `ext_len` 保持 full
  值，匹配 OpenSSL `binderoffset` / Go crypto/tls `bindersOffset`；并修复 resume
  CH2 的 `psk_key_exchange_modes` 被 `pre_shared_key` 覆盖的 bug。
- **resumption PSK label（commit 5d465b0f）**：BabaSSL/Tongsuo 用非标准的
  `"resumption"` label 派生 resumption PSK（`ssl/statem/statem_srvr.c:4260`），非
  RFC 8446 §7.1 的 `"res psk"`。源码级诊断（编译带 fprintf 的 Tongsuo，对比两端
