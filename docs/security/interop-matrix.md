...
- Finished：SM3 transcript HMAC，双向验证通过
...
go test ./test/ -run TestRFC8998_Tongsuo_HandshakeInterop -v
```

## 已知差距

- **1-RTT 应用数据双向 echo**（client→server + server→client application
  traffic secret 的 SM4-GCM 往返）尚未在 TCP harness 中跑通；Tongsuo
  `s_server` 的 `-rev`/`-www` echo 模式与本 harness 的应用层驱动未对齐。
  该方向在 QUIC 端到端测试（`quicgm`）中已覆盖（相同 AEAD + HKDF-SM3 secret
  派生机制）。
