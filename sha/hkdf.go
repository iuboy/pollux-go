...
// HKDF-Extract and HKDF-Expand per RFC 5869, using SHA-256 as the underlying hash.

// hkdfExtract implements HKDF-Extract (RFC 5869 Section 2.2) using SHA-256.
...
// HKDF implements the full HKDF (Extract+Expand) per RFC 5869 using SHA-256.
func HKDF(salt, ikm, info []byte, length int) ([]byte, error) {
	if length <= 0 {
		return nil, errors.New("sha/hkdf: length must be positive")
	}
